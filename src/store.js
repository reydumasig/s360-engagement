...
// Swap load/save/subscribe internals to wire a real backend later.
// =====================================================================

import { ENGAGEMENT_SEED } from './engagement-data';
...
export const ME = PEOPLE[pickIdentity()] || PEOPLE.rey;

// ---- Persistence ----------------------------------------------------
function deepSeed() {
  return { data: JSON.parse(JSON.stringify(ENGAGEMENT_SEED)), meta: {}, version: 1 };
}
...
function startNewWeek() {
  ...
  if (d.client) {
    d.client.currentWeek = Math.min(
      (d.client.currentWeek || 1) + 1,
      d.client.totalWeeks || d.client.currentWeek + 1
    );
  }
